...
stringToWire : String -> Wire
stringToWire =
    String.split ","
        >> List.filterMap stringToOp
        >> List.foldl operate
            { wire = Dict.empty
            , point = ( 0, 0, 0 )
            }
...
addPointToWire : Point -> Wire -> Wire
addPointToWire ( x, y, l ) soFar =
    if Dict.member ( x, y ) soFar then
        soFar

    else
        Dict.insert ( x, y ) l soFar


operate : Op -> Turtle -> Turtle
operate op turtle =
    { wire = toPoints turtle.point op |> List.foldl addPointToWire turtle.wire
    , point = moveTurtle turtle.point op
    }
...
toPoints : Point -> Op -> List Point
toPoints ( x, y, l ) op =
    case op of
        L d ->
            List.map2 (toBackPair y) (range x (0 - d)) (range l d)

        R d ->
            List.map2 (toBackPair y) (range x d) (range l d)

        U d ->
            List.map2 (toPair x) (range y d) (range l d)

        D d ->
            List.map2 (toPair x) (range y (0 - d)) (range l d)


moveTurtle : Point -> Op -> Point
moveTurtle ( x, y, l ) op =
    case op of
        L d ->
            ( x - d, y, l + d )
...
stringToOp s =
    case ( String.left 1 s, String.dropLeft 1 s |> String.toInt ) of
        ( _, Nothing ) ->
            Nothing
...
distances : Wire -> Wire -> ( Int, Int ) -> Maybe Intersection
distances one two ( x, y ) =
    case ( Dict.get ( x, y ) one, Dict.get ( x, y ) two ) of
        ( Just l, Just ll ) ->
            Just
                { manhattan = x + y
                , length = l + ll
...
intersect : Wire -> Wire -> List Intersection
intersect one two =
    Set.intersect (points one) (points two)
        |> Set.toList
        |> Debug.log "intersections"
        |> List.filterMap (distances one two)
        |> Debug.log "intersections with distances"
