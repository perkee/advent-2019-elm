...
updateMany : Msg -> List Model -> ( List Model, Cmd Msg )
updateMany msg models =
    ( List.map (update (Go <| Time.millisToPosix 0)) models, Cmd.none )
                ...
                19690720 ->
                    H.div
                        [ HA.css
                            ...
                            , Css.lineHeight <| Css.rem 10
                            , Css.textAlign <| Css.center
                            , Css.backgroundColor <| Css.rgb 255 1 1
                            ]
                        ]
                        [ H.text <| "won: " ++ String.fromInt val ]

                _ ->
                    H.text <| "lost: " ++ String.fromInt val
...
recurse : ( List Model, Cmd Msg ) -> List Model
recurse ( models, _ ) =
    -- this does nothing
    if modelsCanContinue models then
        updateMany (Go <| Time.millisToPosix 0) models |> recurse
...
subscriptions : List Model -> Sub Msg
subscriptions _ =
    Time.every 100 Go


main : Program () (List Model) Msg
main =
    Browser.element
        { init = \() -> ( List.map makeModel <| possibilitities 8, Cmd.none )
        , view = viewMany >> H.toUnstyled
        , update = updateMany
